...
function print_help_and_quit {
   echo "Usage: path_to_tested_freezoned path_to_reference_freezoned path_to_test_blockchain_directory"
   echo "       path_to_reference_blockchain_directory number_of_blocks_to_replay [number_of_jobs [--dont-copy-config]]"
   echo "Example: ~/work/freezone/freezone/build/Release/programs/freezoned/freezoned ~/master/freezone/freezone/build/Release/programs/freezoned/freezoned ~/freezone/freezone/work1 ~/freezone/freezone/work2 5000000"
...
function run_test_group {
   ...
   echo Running ./$GROUP_TEST_SCRIPT $JOBS $TEST_freezoneD_PATH $REF_freezoneD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   ./$GROUP_TEST_SCRIPT $JOBS $TEST_freezoneD_PATH $REF_freezoneD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   EXIT_CODE=$?
   if [ $EXIT_CODE -ne 0 ]
   then
      EXIT_CODE=-1
...
check_freezoned_path $TEST_freezoneD_PATH
check_freezoned_path $REF_freezoneD_PATH
