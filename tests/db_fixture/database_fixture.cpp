...
#include <freezone/utilities/database_configuration.hpp>
...
#include <freezone/chain/SST_objects/nai_pool_object.hpp>
...
uint32_t freezone_TESTING_GENESIS_TIMESTAMP = 1431700000;

using namespace freezone::plugins::webserver;
using namespace freezone::plugins::database_api;
using namespace freezone::plugins::block_api;
using freezone::plugins::condenser_api::condenser_api_plugin;
...
clean_database_fixture::clean_database_fixture( uint16_t shared_file_size_in_mb )
{
   ...
   appbase::app().register_plugin< freezone::plugins::account_history::account_history_plugin >();
   db_plugin = &appbase::app().register_plugin< freezone::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< freezone::plugins::rc::rc_plugin >();
   appbase::app().register_plugin< freezone::plugins::witness::witness_plugin >();

   db_plugin->logging = false;
   appbase::app().initialize<
      freezone::plugins::account_history::account_history_plugin,
   ...
   generate_block();
   db->set_hardfork( freezone_BLOCKCHAIN_VERSION.minor_v() );
   generate_block();
...
void clean_database_fixture::validate_database()
{
   ...
   appbase::app().get_plugin< freezone::plugins::rc::rc_plugin >().validate_database();
}
...
void clean_database_fixture::resize_shared_mem( uint64_t size )
{
...

   {
      ...
      args.database_cfg = freezone::utilities::default_database_configuration();
      db->open( args );
   }
...
asset_symbol_type database_fixture::get_new_SST_symbol( uint8_t token_decimal_places, chain::database* db )
{
   // The list of available nais is not dependent on SST desired precision (token_decimal_places).
...
void database_fixture::open_database( uint16_t shared_file_size_in_mb )
{
   if( !data_dir )
   {
      data_dir = fc::temp_directory( freezone::utilities::temp_directory_path() );
...
{
   skip |= default_skip;
   db_plugin->debug_generate_blocks( freezone::utilities::key_to_wif( key ), 1, skip, miss_blocks );
}
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db->head_block_time() - timestamp ).to_seconds() < freezone_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      ...
      op.creator = creator;
      op.fee = asset( actual_fee, freezone_SYMBOL );
      ...
      trx.set_expiration( db->head_block_time() + freezone_MAX_TIME_UNTIL_EXPIRATION );
      ...
      if( fee_remainder > 0 )
      {
         vest( freezone_INIT_MINER_NAME, name, asset( fee_remainder, freezone_SYMBOL ) );
      }
...
)
{
   try
   {
      return account_create(
         name,
         freezone_INIT_MINER_NAME,
         init_account_priv_key,
         std::max( db->get_witness_schedule_object().median_props.account_creation_fee.amount * freezone_CREATE_ACCOUNT_WITH_freezone_MODIFIER, share_type( 100 ) ),
         key,
         post_key,
         "" );
   }
   ...
   const share_type& fee )
{
   try
   {
      ...
      op.fee = asset( fee, freezone_SYMBOL );
   ...
   )
{
   try
   {
      transfer( freezone_INIT_MINER_NAME, account_name, asset( amount, freezone_SYMBOL ) );
   ...
   )
{
   try
   {
      ...
      {
         if( amount.symbol.space() == asset_symbol_type::SST_nai_space )
         {
            db.adjust_balance(account_name, amount);
            db.adjust_supply(amount);
            // Note that SST have no equivalent of SBD, hence no virtual supply, hence no need to update it.
            return;
         }

         db.modify( db.get_account( account_name ), [&]( account_object& a )
         {
            if( amount.symbol == freezone_SYMBOL )
               a.balance += amount;
         ...
         if( amount.symbol == SBD_SYMBOL )
         {
            ...
            if( median_feed.current_median_history.is_null() )
               ...
               {
                  f.current_median_history = price( asset( 1, SBD_SYMBOL ), asset( 1, freezone_SYMBOL ) );
               });
         }
   ...
   const asset& amount )
{
   try
   {
      ...
      {
         db->adjust_balance( account_name, -amount );
         db->adjust_balance( account_name, db->to_freezone( amount ) );
         db->adjust_supply( -amount );
         db->adjust_supply( db->to_freezone( amount ) );
      }
   } FC_CAPTURE_AND_RETHROW( (account_name)(amount) )
}
   ...
   const asset& amount )
{
   try
   {
      ...
      if( from == freezone_INIT_MINER_NAME )
      {
...
void database_fixture::vest( const string& from, const string& to, const asset& amount )
{
   try
   {
      if ( amount.symbol.space() == asset_symbol_type::legacy_space )
         FC_ASSERT( amount.symbol == freezone_SYMBOL, "Can only vest TESTS" );
      else
...
{
   vest( from, asset( amount, freezone_SYMBOL ) );
}
...
void database_fixture::set_price_feed( const price& new_price )
{
   for( size_t i = 1; i < 8; i++ )
   {
      ...
      op.owner = freezone_INIT_MINER_NAME + fc::to_string( i );
   ...
   generate_blocks( freezone_BLOCKS_PER_HOUR );
...
{
   trx.clear();
   for( size_t i=0; i<freezone_MAX_WITNESSES; i++ )
   {
      witness_set_properties_operation op;
      op.owner = freezone_INIT_MINER_NAME + (i == 0 ? "" : fc::to_string( i ));
      op.props = props;
   ...
   for( size_t i=0; i<2*freezone_MAX_WITNESSES+1; i++ )
   {
      generate_block();
   ...
   FC_ASSERT( false, "Couldn't apply properties in ${n} blocks", ("n", 2*freezone_MAX_WITNESSES+1) );
}
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   while( itr != acc_hist_idx.begin() && ops.size() < num_ops )
   {
      ...
      ops.push_back( fc::raw::unpack_from_vector< freezone::chain::operation >( serialized_op ) );
   }
...
void database_fixture::validate_database()
{
   try
   {
      ...
      db->validate_SST_invariants();
   }
   FC_LOG_AND_RETHROW();
}

asset_symbol_type database_fixture::create_SST_with_nai( const string& account_name, const fc::ecc::private_key& key,
   uint32_t nai, uint8_t token_decimal_places )
{
   ...
   try
   {
      ...
      op.SST_creation_fee = this->db->get_dynamic_global_properties().SST_creation_fee;
...
asset_symbol_type database_fixture::create_SST( const string& account_name, const fc::ecc::private_key& key,
   uint8_t token_decimal_places )
{
   asset_symbol_type symbol;
   try
   {
      auto nai_symbol = this->get_new_SST_symbol( token_decimal_places, this->db );
      symbol = create_SST_with_nai( account_name, key, nai_symbol.to_nai(), token_decimal_places );
   }
   FC_LOG_AND_RETHROW();
...
void sub_set_create_op( SST_create_operation* op, account_name_type control_acount, chain::database& db )
{
   op->precision = op->symbol.decimals();
...
void set_create_op( SST_create_operation* op, account_name_type control_account, uint8_t token_decimal_places, chain::database& db )
{
   op->symbol = database_fixture::get_new_SST_symbol( token_decimal_places, &db );
   sub_set_create_op( op, control_account, db );
}

void set_create_op( SST_create_operation* op, account_name_type control_account, uint32_t token_nai, uint8_t token_decimal_places, chain::database& db )
{
   op->symbol.from_nai(token_nai, token_decimal_places);
   sub_set_create_op( op, control_account, db );
}

std::array<asset_symbol_type, 3> database_fixture::create_SST_3(const char* control_account_name, const fc::ecc::private_key& key)
{
   SST_create_operation op0;
   SST_create_operation op1;
...
void push_invalid_operation(const operation& invalid_op, const fc::ecc::private_key& key, database* db)
{
   ...
   freezone_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
}

void database_fixture::create_invalid_SST( const char* control_account_name, const fc::ecc::private_key& key )
{
   // Fail due to precision too big.
   SST_create_operation op_precision;
   freezone_REQUIRE_THROW( set_create_op( &op_precision, control_account_name, freezone_ASSET_MAX_DECIMALS + 1, *this->db ), fc::assert_exception );
}

void database_fixture::create_conflicting_SST( const asset_symbol_type existing_SST, const char* control_account_name,
   const fc::ecc::private_key& key )
{
   // Fail due to the same nai & precision.
   SST_create_operation op_same;
   set_create_op( &op_same, control_account_name, existing_SST.to_nai(), existing_SST.decimals(), *this->db );
   push_invalid_operation( op_same, key, this->db );
   // Fail due to the same nai (though different precision).
   SST_create_operation op_same_nai;
   set_create_op( &op_same_nai, control_account_name, existing_SST.to_nai(), existing_SST.decimals() == 0 ? 1 : 0, *this->db );
...
uint64_t sps_proposal_database_fixture::get_nr_blocks_until_maintenance_block()
{
   ...
   auto ret = ( next_maintenance_time - block_time ).to_seconds() / freezone_BLOCK_INTERVAL;
...
json_rpc_database_fixture::json_rpc_database_fixture()
{
   ...
   db_plugin = &appbase::app().register_plugin< freezone::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< freezone::plugins::witness::witness_plugin >();
   rpc_plugin = &appbase::app().register_plugin< freezone::plugins::json_rpc::json_rpc_plugin >();
   appbase::app().register_plugin< freezone::plugins::block_api::block_api_plugin >();
   appbase::app().register_plugin< freezone::plugins::database_api::database_api_plugin >();
   appbase::app().register_plugin< freezone::plugins::condenser_api::condenser_api_plugin >();
   ...
   appbase::app().initialize<
      ...
      freezone::plugins::block_api::block_api_plugin,
      freezone::plugins::database_api::database_api_plugin,
      freezone::plugins::condenser_api::condenser_api_plugin
      >( argc, argv );

   appbase::app().get_plugin< freezone::plugins::condenser_api::condenser_api_plugin >().plugin_startup();
