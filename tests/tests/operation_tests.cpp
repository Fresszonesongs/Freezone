...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( freezone_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = freezone_INIT_MINER_NAME;
      ...
      op.fee = asset( 101, freezone_SYMBOL );
      ...
      op.fee = asset( 100, freezone_SYMBOL );
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      tx.validate();
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( freezone_INIT_MINER_NAME ).balance.amount + 1, freezone_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      ...
      fund( freezone_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( freezone_INIT_MINER_NAME, freezone_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = freezone_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / freezone_BLOCK_INTERVAL );
      ...
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.signatures.clear();
      sign( tx, bob_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( freezone_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + freezone_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / freezone_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = freezone::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + freezone_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / freezone_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      vest( freezone_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      vote.author = "alice";
      vote.permlink = "test1";
      vote.weight = freezone_100_PERCENT;
      tx.operations.push_back( comment );
      tx.operations.push_back( vote );
      tx.set_expiration( db->head_block_time() + freezone_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_block();
      vote.weight = -1 * freezone_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      generate_blocks( freezone_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / freezone_BLOCK_INTERVAL );
      ...
      generate_blocks( freezone_CASHOUT_WINDOW_SECONDS / freezone_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().target_votes_per_period * freezone_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_mana - alice.voting_manabar.current_mana ) - freezone_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + freezone_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_manabar.current_mana ) - freezone_VOTE_DUST_THRESHOLD );
         ...
         util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), freezone_VOTING_MANA_REGENERATION_SECONDS );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana ) - freezone_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + freezone_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_manabar.current_mana ) - freezone_VOTE_DUST_THRESHOLD );
         ...
         op.weight = -1 * freezone_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         util::manabar_params downvote_params( util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4, freezone_VOTING_MANA_REGENERATION_SECONDS );
         old_downvote_manabar.regenerate_mana( downvote_params, db->head_block_time() );
         int64_t sam_weight = old_downvote_manabar.current_mana - db->get_account( "sam" ).downvote_manabar.current_mana - freezone_VOTE_DUST_THRESHOLD;
         ...
         int64_t regenerated_power = (freezone_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / freezone_VOTING_MANA_REGENERATION_SECONDS;
         ...
         int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / freezone_100_PERCENT ).to_uint64() - freezone_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( freezone_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / freezone_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = ( ( uint128_t( old_manabar.current_mana ) * 25 * freezone_1_PERCENT + votes_per_period ) / ( votes_per_period * freezone_100_PERCENT ) ).to_int64() - freezone_VOTE_DUST_THRESHOLD;

         op.voter = "alice";
         op.weight = freezone_1_PERCENT * 25;
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         tx.signatures.clear();
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( freezone_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / freezone_100_PERCENT;
         ...
         op.weight = freezone_1_PERCENT * -75;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         sign( tx, alice_private_key );
         ...
         new_rshares = old_downvote_manabar.current_mana - db->get_account( "alice" ).downvote_manabar.current_mana - freezone_VOTE_DUST_THRESHOLD;
         ...
         op.weight = -1 * freezone_100_PERCENT;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         sign( tx, alice_private_key );
...

         {
            const auto& bob_comment = db->get_comment( "bob", string( "foo" ) );
            const auto& alice = db->get_account( "alice" );
            BOOST_REQUIRE( bob_comment.net_rshares == old_net_rshares - alice_weight + freezone_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( bob_comment.abs_rshares == old_abs_rshares + alice_weight - freezone_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( alice_bob_vote->rshares == -1 * ( alice_weight - freezone_VOTE_DUST_THRESHOLD ) );
            BOOST_REQUIRE( alice_bob_vote->last_update == db->head_block_time() );
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - freezone_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + freezone_BLOCK_INTERVAL ), true );
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - freezone_VOTE_DUST_THRESHOLD;
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when increasing vote" );
         {
            ...
            op.voter = "sam";
            op.author = "dave";
            op.weight = 75 * freezone_1_PERCENT;
            tx.clear();
            tx.operations.push_back( comment_op );
            tx.operations.push_back( op );
            sign( tx, sam_private_key );
            ...
            auto old_rshares = db->get< comment_vote_object, by_voter_comment_symbol >( boost::make_tuple( sam.id, db->get_comment( op.author, op.permlink ).id, freezone_SYMBOL ) ).rshares;
            ...
            new_rshares = ( ( uint128_t( old_manabar.current_mana ) * op.weight + votes_per_period - 1 ) / ( votes_per_period * freezone_100_PERCENT ) ).to_int64() - freezone_VOTE_DUST_THRESHOLD;
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when decreasing vote" );
         {
            ...
            op.weight = 50 * freezone_1_PERCENT;
            tx.clear();
            tx.operations.push_back( op );
            sign( tx, sam_private_key );
            db->push_transaction( tx, 0 );
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when changing to downvote" );
         {
            ...
            new_rshares = ( ( uint128_t( old_manabar.current_mana ) * 50 * freezone_1_PERCENT + votes_per_period - 1 ) / ( votes_per_period * freezone_100_PERCENT ) ).to_int64();
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when increasing downvote" );
         {
            ...
            new_rshares = ( ( uint128_t( old_manabar.current_mana ) * freezone_100_PERCENT + votes_per_period - 1 ) / ( votes_per_period * freezone_100_PERCENT ) ).to_int64();
            auto delta = new_rshares - ( std::abs( old_rshares ) + freezone_VOTE_DUST_THRESHOLD );
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when decreasing downvote" );
         {
            ...
            op.weight = -25 * freezone_1_PERCENT;
            tx.clear();
            tx.operations.push_back( op );
            sign( tx, sam_private_key );
            db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure transfering freezone to freezone.dao" );
      op.from = "bob";
      op.to = freezone_TREASURY_ACCOUNT;
      op.amount = ASSET( "1.000 TESTS" );
      tx.signatures.clear();
      tx.operations.clear();
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx ), fc::exception );

      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( freezone_TREASURY_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test transfering SBD to freezone.dao" );
      auto treasury_sbd_balance = db->get_account( freezone_TREASURY_ACCOUNT ).sbd_balance;
      ...
      BOOST_REQUIRE( db->get_account( freezone_TREASURY_ACCOUNT ).sbd_balance == treasury_sbd_balance + ASSET( "1.000 TBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_freezone.amount, freezone_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_freezone.amount.value == vests.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, freezone_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + freezone_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_validate )
{
   ...
   op.vesting_shares = asset( 10, freezone_SYMBOL );
   BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( freezone_VESTING_WITHDRAW_INTERVALS * 2 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + freezone_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( freezone_VESTING_WITHDRAW_INTERVALS * 3 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + freezone_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_freezone;
            gpo.total_vesting_fund_freezone = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_freezone_asset::from_asset( asset(freezone_MIN_ACCOUNT_CREATION_FEE + 10, freezone_SYMBOL) );
      op.props.maximum_block_size = freezone_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == freezone_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == freezone_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == freezone_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = freezone_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == freezone_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = freezone_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( db->get_witness( freezone_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( freezone_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( custom_json_rate_limit )
{
   try
   {
      ...
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), plugin_exception );
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 TBD" ), ASSET( "1000.000 TESTS" ) ); // 1000 freezone : 1 SBD
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + freezone_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      freezone_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
//      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( SBD_SYMBOL, freezone_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        freezone_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        freezone_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        freezone_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      vest( freezone_INIT_MINER_NAME, "bob", asset( 1000, freezone_SYMBOL ) );
      ...
      generate_blocks( db->head_block_time() + freezone_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + freezone_OWNER_UPDATE_LIMIT + fc::seconds( freezone_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + freezone_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - freezone_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( freezone_OWNER_AUTH_RECOVERY_PERIOD - freezone_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      freezone_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      freezone_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      freezone_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + freezone_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( freezone_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      freezone_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      freezone_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.freezone_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      op.fee = ASSET( "0.100 TESTS" );
      op.json_meta = "";
      ...
      op.sbd_amount.symbol = freezone_SYMBOL;
      freezone_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when freezone symbol != freezone" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.freezone_amount.symbol = SBD_SYMBOL;
      freezone_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != SBD and fee symbol != freezone" );
      op.freezone_amount.symbol = freezone_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      freezone_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd == 0 and freezone == 0" );
      op.fee.symbol = freezone_SYMBOL;
      op.sbd_amount.amount = 0;
      op.freezone_amount.amount = 0;
      freezone_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd < 0" );
      op.sbd_amount.amount = -100;
      op.freezone_amount.amount = 1000;
      freezone_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when freezone < 0" );
      op.sbd_amount.amount = 1000;
      op.freezone_amount.amount = -100;
      freezone_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      op.sbd_amount.amount = 0;
      op.freezone_amount.amount = 10000;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      auto alice_freezone_balance = alice.balance - op.freezone_amount - op.fee;
      auto alice_sbd_balance = alice.sbd_balance - op.sbd_amount;
      auto bob_freezone_balance = bob.balance;
      auto bob_sbd_balance = bob.sbd_balance;
      auto sam_freezone_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.freezone_balance == op.freezone_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_freezone_balance );
      BOOST_REQUIRE( alice.sbd_balance == alice_sbd_balance );
      BOOST_REQUIRE( bob.balance == bob_freezone_balance );
      BOOST_REQUIRE( bob.sbd_balance == bob_sbd_balance );
      BOOST_REQUIRE( sam.balance == sam_freezone_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.freezone_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.freezone_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + freezone_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      freezone_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + freezone_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.freezone_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + freezone_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * freezone_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.freezone_balance == et_op.freezone_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when freezone == 0 and sbd == 0" );
      op.sbd_amount.amount = 0;
      freezone_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when freezone is not freezone symbol" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.freezone_amount = ASSET( "1.000 TBD" );
      freezone_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.freezone_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).freezone_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).freezone_balance == ASSET( "0.800 TESTS" ) );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less freezone than available" );
      op.freezone_amount = ASSET( "0.000 TESTS" );
      op.sbd_amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).freezone_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).freezone_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).freezone_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.freezone_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      freezone_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).freezone_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).freezone_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.freezone_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is freezone" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Failure when transferring to freezone.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring freezone to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring freezone to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Failure withdrawing TESTS to freezone.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- Success withdrawing TBD to freezone.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing freezone to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id++;
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + freezone_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing freezone to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      generate_blocks( db->head_block_time() + freezone_SAVINGS_WITHDRAW_TIME - fc::seconds( freezone_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < freezone_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = freezone_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == freezone_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + freezone_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      db->get< comment_vote_object, by_comment_voter_symbol >( boost::make_tuple( db->get_comment( "alice", string( "test" ) ).id, db->get_account( "alice" ).id, freezone_SYMBOL )  );
      ...
      vote.weight = freezone_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      sign( tx, alice_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_freezone = ASSET( "0.000 TESTS" );
      ...
      op.reward_freezone.amount = 1000;
      op.validate();

      op.reward_freezone.amount = 0;
      ...
      BOOST_TEST_MESSAGE( "Testing wrong freezone symbol" );
      op.reward_freezone = ASSET( "1.000 TBD" );
      freezone_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong SBD symbol" );
      op.reward_freezone = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Attempting to claim more freezone than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_freezone = ASSET( "20.000 TESTS" );
      op.reward_sbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_freezone + op.reward_freezone );
      ...
      op.reward_freezone = ASSET( "10.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_authorities )
{
   try
   {
      ...
      signed_transaction tx;
      ACTORS( (alice)(bob) )
      vest( freezone_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      auto delegation = db->find< vesting_delegation_object, by_delegation >( boost::make_tuple( op.delegator, op.delegatee, freezone_SYMBOL ) );
      ...
      vote_op.weight = freezone_100_PERCENT;
      tx.set_expiration( db->head_block_time() + freezone_MAX_TIME_UNTIL_EXPIRATION );
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - freezone_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - freezone_VOTE_DUST_THRESHOLD );

      generate_block();
      ACTORS( (sam)(dave) )
      generate_block();

      vest( freezone_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
      ...
      util::manabar_params sam_params( util::get_effective_vesting_shares( db->get_account( "sam" ) ), freezone_VOTING_MANA_REGENERATION_SECONDS );
      util::manabar_params dave_params( util::get_effective_vesting_shares( db->get_account( "dave" ) ), freezone_VOTING_MANA_REGENERATION_SECONDS );
      ...
      delegation = db->find< vesting_delegation_object, by_delegation >( boost::make_tuple( op.delegator, op.delegatee, freezone_SYMBOL ) );
      BOOST_REQUIRE( delegation == nullptr );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get_account( freezone_TREASURY_ACCOUNT ), [=]( account_object& a )
         {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), freezone_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( freezone_INIT_MINER_NAME + fc::to_string( i ) ), freezone_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), freezone_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * freezone_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * freezone_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( freezone_TREASURY_ACCOUNT ), 10 * freezone_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * freezone_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      freezone_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - freezone_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_freezone;
            gpo.total_reward_fund_freezone = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_freezone;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_freezone.amount + db->get_account( "bob" ).reward_sbd_balance.amount + db->get_account( "sam" ).reward_vesting_freezone.amount + db->get_account( "sam" ).reward_sbd_balance.amount + db->get_account( freezone_TREASURY_ACCOUNT ).sbd_balance.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_sbd_balance.amount + db->get_account( "alice" ).reward_vesting_freezone.amount + db->get_account( freezone_TREASURY_ACCOUNT ).sbd_balance.amount ) == db->get_account( "bob" ).reward_vesting_freezone.amount + db->get_account( "bob" ).reward_sbd_balance.amount + 1 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_sbd_balance.amount + db->get_account( "alice" ).reward_vesting_freezone.amount + db->get_account( freezone_TREASURY_ACCOUNT ).sbd_balance.amount ) == ( db->get_account( "sam" ).reward_vesting_freezone.amount + db->get_account( "sam" ).reward_sbd_balance.amount ) / 2 + 1 );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_freezone.amount == db->get_account( "bob" ).reward_sbd_balance.amount + 1 );
      BOOST_REQUIRE( db->get_account( "sam" ).reward_vesting_freezone.amount == db->get_account( "sam" ).reward_sbd_balance.amount + 1 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.owner = "alice";
      freezone_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below freezone_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( freezone_MIN_BLOCK_SIZE_LIMIT - 1 );
      freezone_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting sbd_interest_rate to freezone_100_PERCENT + 1" );
      prop_op.props[ "sbd_interest_rate" ].clear();
      prop_op.props[ "sbd_interest_rate" ] = fc::raw::pack_to_vector( freezone_100_PERCENT + 1 );
      freezone_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new sbd_exchange_rate with SBD / freezone" );
      ...
      uint64_t unit = uint64_t(1) << freezone_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / freezone_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / freezone_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/freezone_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, freezone_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == freezone_MIN_BLOCK_SIZE_LIMIT + 1 );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( freezone_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == freezone_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << freezone_RD_DECAY_DENOM_SHIFT ) / freezone_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( freezone_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( freezone_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - freezone_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - freezone_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> freezone_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( freezone_TEMP_ACCOUNT ), [&]( account_object& a )
         {
...
BOOST_AUTO_TEST_CASE( account_auth_tests )
{
   try
   {
      ...
      vest( freezone_INIT_MINER_NAME, "alice" , ASSET( "10.000 TESTS" ) );
      vest( freezone_INIT_MINER_NAME, "bob" , ASSET( "10.000 TESTS" ) );
      vest( freezone_INIT_MINER_NAME, "charlie" , ASSET( "10.000 TESTS" ) );
      generate_block();
