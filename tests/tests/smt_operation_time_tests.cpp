...
BOOST_FIXTURE_TEST_SUITE( SST_operation_time_tests, clean_database_fixture )

BOOST_AUTO_TEST_CASE( SST_refunds )
{
   try
   {
      ...
      auto bobs_balance  = asset( 1000000, freezone_SYMBOL );
      auto sams_balance  = asset( 800000, freezone_SYMBOL );
      auto daves_balance = asset( 600000, freezone_SYMBOL );
      ...
      setup_op.freezone_satoshi_min       = 2400001;
      ...
      BOOST_REQUIRE( token.phase == SST_phase::setup_completed );

      generate_block();

      BOOST_REQUIRE( token.phase == SST_phase::ico );

      BOOST_TEST_MESSAGE( " --- SST contributions" );
      uint32_t num_contributions = 0;
      for ( uint64_t i = 0; i < contribution_window_blocks; i++ )
      {
         SST_contribute_operation contrib_op;

         contrib_op.symbol = symbol;
         contrib_op.contribution_id = i;

         contrib_op.contributor = "bob";
         contrib_op.contribution = asset( bobs_balance.amount / contribution_window_blocks, freezone_SYMBOL );
         ...
         contrib_op.contribution = asset( sams_balance.amount / contribution_window_blocks, freezone_SYMBOL );
         ...
         contrib_op.contribution = asset( daves_balance.amount / contribution_window_blocks, freezone_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_balance( "bob", freezone_SYMBOL ) == asset( 0, freezone_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( "sam", freezone_SYMBOL ) == asset( 0, freezone_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( "dave", freezone_SYMBOL ) == asset( 0, freezone_SYMBOL ) );

      generate_block();

      BOOST_REQUIRE( token.phase == SST_phase::launch_failed );
      ...
      BOOST_REQUIRE( db->get_balance( "bob", freezone_SYMBOL ) == bobs_balance );
      BOOST_REQUIRE( db->get_balance( "sam", freezone_SYMBOL ) == sams_balance );
      BOOST_REQUIRE( db->get_balance( "dave", freezone_SYMBOL ) == daves_balance );

      validate_database();

      auto& ico_idx = db->get_index< SST_ico_index, by_symbol >();
      BOOST_REQUIRE( ico_idx.find( symbol ) == ico_idx.end() );
      auto& contribution_idx = db->get_index< SST_contribution_index, by_symbol_id >();
      BOOST_REQUIRE( contribution_idx.find( boost::make_tuple( symbol, 0 ) ) == contribution_idx.end() );
   }
   FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( SST_ico_payouts )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing SST ICO payouts" );
      ACTORS( (creator)(alice)(bob)(charlie)(dan)(elaine)(fred)(george)(henry) )

      generate_block();

      auto alices_balance    = asset( 5000000, freezone_SYMBOL );
      auto bobs_balance      = asset( 25000000, freezone_SYMBOL );
      auto charlies_balance  = asset( 10000000, freezone_SYMBOL );
      auto dans_balance      = asset( 25000000, freezone_SYMBOL );
      auto elaines_balance   = asset( 60000000, freezone_SYMBOL );
      auto freds_balance     = asset( 0, freezone_SYMBOL );
      auto georges_balance   = asset( 0, freezone_SYMBOL );
      auto henrys_balance    = asset( 0, freezone_SYMBOL );
      ...
      {
         FUND( e.first, asset( std::get< 0 >( e.second ), freezone_SYMBOL ) );
      }
      ...
      SST_capped_generation_policy capped_generation_policy;
      capped_generation_policy.generation_unit.freezone_unit[ "fred" ] = 3;
      capped_generation_policy.generation_unit.freezone_unit[ "george" ] = 2;

      capped_generation_policy.generation_unit.token_unit[ SST_DESTINATION_FROM ] = 7;
      ...
      ico_tier_op1.symbol = symbol;
      ico_tier_op1.generation_policy = capped_generation_policy;
      ico_tier_op1.freezone_satoshi_cap = 100000000;

      SST_setup_ico_tier_operation ico_tier_op2;
      ico_tier_op2.control_account = "creator";
      ico_tier_op2.symbol = symbol;
      ico_tier_op2.generation_policy = capped_generation_policy;
      ico_tier_op2.freezone_satoshi_cap = 150000000;
      ...
      for ( auto& e : contributor_contributions )
      {
         ...
         contrib_op.contribution = asset( std::get< 0 >( e.second ), freezone_SYMBOL );
      ...
      BOOST_REQUIRE( token.phase == SST_phase::launch_success );
      ...
      BOOST_REQUIRE( db->get_balance( "alice", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "bob", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "charlie", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "dan", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "elaine", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "fred", freezone_SYMBOL ).amount == 75000000 );
      BOOST_REQUIRE( db->get_balance( "george", freezone_SYMBOL ).amount == 50000000 );
      BOOST_REQUIRE( db->get_balance( "henry", freezone_SYMBOL ).amount == 0 );
...
BOOST_AUTO_TEST_CASE( SST_ico_payouts_special_destinations )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing SST ICO payouts special destinations" );
      ACTORS( (creator)(alice)(bob)(charlie)(dan)(elaine)(fred)(george)(henry) )
      ...
      SST_capped_generation_policy capped_generation_policy1;
      capped_generation_policy1.generation_unit.freezone_unit[ SST_DESTINATION_MARKET_MAKER ] = 3;
      capped_generation_policy1.generation_unit.freezone_unit[ "george" ] = 2;

      capped_generation_policy1.generation_unit.token_unit[ SST_DESTINATION_FROM ] = 7;
      capped_generation_policy1.generation_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] = 1;
      capped_generation_policy1.generation_unit.token_unit[ SST_DESTINATION_REWARDS ] = 2;
      ...
      SST_capped_generation_policy capped_generation_policy2;
      capped_generation_policy2.generation_unit.freezone_unit[ SST_DESTINATION_MARKET_MAKER ] = 3;
      capped_generation_policy2.generation_unit.freezone_unit[ SST_DESTINATION_ACCOUNT_VESTING( "george" ) ] = 2;

      capped_generation_policy2.generation_unit.token_unit[ SST_DESTINATION_FROM ] = 5;
      capped_generation_policy2.generation_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] = 1;
      capped_generation_policy2.generation_unit.token_unit[ SST_DESTINATION_REWARDS ] = 2;
      capped_generation_policy2.generation_unit.token_unit[ SST_DESTINATION_ACCOUNT_VESTING( "george" ) ] = 2;
      ...
      BOOST_REQUIRE( db->get_balance( "dan", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "elaine", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "fred", freezone_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "george", freezone_SYMBOL ).amount == 40000000 );
      BOOST_REQUIRE( db->get_balance( "henry", freezone_SYMBOL ).amount == 0 );
      ...
      BOOST_REQUIRE( token.market_maker.freezone_balance == asset( 75000000, freezone_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( SST_vesting_withdrawals )
{
   BOOST_TEST_MESSAGE( "Testing: SST vesting withdrawals" );
   ...
   auto next_withdrawal = db->head_block_time() + SST_VESTING_WITHDRAW_INTERVAL_SECONDS;
   ...
   {
      generate_blocks( db->head_block_time() + SST_VESTING_WITHDRAW_INTERVAL_SECONDS );
      ...
      if ( i == freezone_VESTING_WITHDRAW_INTERVALS - 1 )
         ...
         BOOST_REQUIRE( balance_obj->next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + SST_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
   ...
   generate_blocks( db->head_block_time() + SST_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto charlie_symbol = create_SST( "charlie", charlie_private_key, 3 );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get< SST_token_object, by_symbol >( alice_symbol ), [=]( SST_token_object& SST )
         {
            ...
            SST.current_supply = 100000;
            SST.total_vesting_shares = alice_vests.amount;
            SST.total_vesting_fund_SST = 100000;
            SST.votes_per_regeneration_period = 50;
            SST.vote_regeneration_period_seconds = 5*24*60*60;
            SST.recent_claims = uint128_t( 0, alice_recent_claims );
            SST.last_reward_update = now;
         });
         ...
         db.modify( db.get< SST_token_object, by_symbol >( bob_symbol ), [=]( SST_token_object& SST )
         {
            ...
            SST.votes_per_regeneration_period = 50;
            SST.vote_regeneration_period_seconds = 5*24*60*60;
            SST.recent_claims = uint128_t( 0, bob_recent_claims );
            SST.last_reward_update = now;
            SST.author_reward_curve = curve_id::linear;
         });

         db.modify( db.get< SST_token_object, by_symbol >( charlie_symbol ), [=]( SST_token_object& SST )
         {
            SST.phase = SST_phase::launch_success;
            SST.current_supply = 0;
            SST.total_vesting_shares = 0;
            SST.total_vesting_fund_SST = 0;
            SST.votes_per_regeneration_period = 50;
            SST.vote_regeneration_period_seconds = 5*24*60*60;
            SST.recent_claims = uint128_t( 0, charlie_recent_claims );
            SST.last_reward_update = now;
         });
      });
      generate_block();
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.rshares[ freezone_SYMBOL ] = 10000 + freezone_VOTE_DUST_THRESHOLD;
      vote.rshares[ alice_symbol ] = -5000 - freezone_VOTE_DUST_THRESHOLD;
      ...
      vote.rshares.clear();
      vote.rshares[ freezone_SYMBOL ] = 10000 + freezone_VOTE_DUST_THRESHOLD;
      vote.rshares[ bob_symbol ] = 5000 + freezone_VOTE_DUST_THRESHOLD;
      tx.clear();
      tx.operations.push_back( comment );
      tx.operations.push_back( comment_opts );
      tx.operations.push_back( vote );
...

      {
         ...
         const auto& alice_rf = db->get< SST_token_object, by_symbol >( alice_symbol );
         ...
         const auto& bob_rf = db->get< SST_token_object, by_symbol >( bob_symbol );
         BOOST_REQUIRE( bob_rf.recent_claims.to_uint64() == bob_recent_claims );
         BOOST_REQUIRE( bob_rf.last_reward_update == last_claims_update );

         const auto& charlie_rf = db->get< SST_token_object, by_symbol >( charlie_symbol );
...

      {
         ...
         bob_recent_claims += util::evaluate_reward_curve(
            vote.rshares[ bob_symbol ] - freezone_VOTE_DUST_THRESHOLD,
...
BOOST_AUTO_TEST_CASE( SST_rewards )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         auto alice_SST_vests = db.create_vesting( db.get_account( "alice" ), asset( 1000000, alice_symbol ), false );
         alice_SST_vests     += db.create_vesting( db.get_account( "bob" ), asset( 1000000, alice_symbol ), false );
         alice_SST_vests     += db.create_vesting( db.get_account( "charlie" ), asset( 1000000, alice_symbol ), false );
         alice_SST_vests     += db.create_vesting( db.get_account( "dave" ), asset( 1000000, alice_symbol ), false );
         auto now = db.head_block_time();

         db.modify( db.get< SST_token_object, by_symbol >( alice_symbol ), [&]( SST_token_object& SST )
         {
            SST.phase = SST_phase::launch_success;
            SST.current_supply = 4000000;
            SST.total_vesting_shares = alice_SST_vests.amount;
            SST.total_vesting_fund_SST = 4000000;
            SST.votes_per_regeneration_period = 50;
            SST.vote_regeneration_period_seconds = 5*24*60*60;
            SST.recent_claims = uint128_t( 0, recent_claims );
            SST.author_reward_curve = curve_id::convergent_linear;
            SST.curation_reward_curve = convergent_square_root;
            SST.content_constant = freezone_CONTENT_CONSTANT_HF21;
            SST.percent_curation_rewards = 50 * freezone_1_PERCENT;
            SST.last_reward_update = now;
         });
      ...
      comment_opts.author = "alice";
      comment_opts.permlink = "test";
      comment_opts.percent_freezone_dollars = 0;
      comment_opts.extensions.insert( ava );
      vote2_operation vote;
      vote.voter = "alice";
      vote.author = comment.author;
      vote.permlink = comment.permlink;
      vote.rshares[ freezone_SYMBOL ] = 100ull * freezone_VOTE_DUST_THRESHOLD;
      vote.rshares[ alice_symbol ] = vote.rshares[ freezone_SYMBOL ];
      ...
      generate_blocks( db->head_block_time() + ( freezone_REVERSE_AUCTION_WINDOW_SECONDS_HF21 / 2 ), true );

      tx.clear();
      vote.voter = "bob";
      vote.rshares[ freezone_SYMBOL ] = 50ull * freezone_VOTE_DUST_THRESHOLD;
      vote.rshares[ alice_symbol ] = vote.rshares[ freezone_SYMBOL ];
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      db->push_transaction( tx, 0 );

      generate_blocks( db->head_block_time() + freezone_REVERSE_AUCTION_WINDOW_SECONDS_HF21, true );
      ...
      generate_blocks( db->get_comment( comment.author, comment.permlink ).cashout_time - ( freezone_UPVOTE_LOCKOUT_SECONDS / 2 ), true );
      ...
      generate_blocks( db->get_comment( comment.author, comment.permlink ).cashout_time - freezone_BLOCK_INTERVAL, true );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            SST.recent_claims = uint128_t( 0, recent_claims );
            SST.reward_balance = asset( reward_fund, alice_symbol );
            SST.current_supply += reward_fund;
            SST.last_reward_update = now;
         });
         ...
         db.modify( db.get( reward_fund_id_type() ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance += asset( reward_delta, freezone_SYMBOL );
            rfo.recent_claims = uint128_t( 0, recent_claims );
            rfo.last_update = now;
         });
         ...
         {
            gpo.current_supply += asset( reward_delta, freezone_SYMBOL );
         });
      });
      ...
      const auto& alice_SST = db->get< SST_token_object, by_symbol >( alice_symbol );

      BOOST_REQUIRE( rf.recent_claims == alice_SST.recent_claims );
      BOOST_REQUIRE( rf.reward_balance.amount == alice_SST.reward_balance.amount );

      const auto& alice_SST_balance = db->get< account_rewards_balance_object, by_name_liquid_symbol >( boost::make_tuple( "alice", alice_symbol ) );
      const auto& alice_reward_balance = db->get_account( "alice" );
      BOOST_REQUIRE( alice_reward_balance.reward_vesting_freezone.amount == alice_SST_balance.pending_vesting_value.amount );

      const auto& bob_SST_balance = db->get< account_rewards_balance_object, by_name_liquid_symbol >( boost::make_tuple( "bob", alice_symbol ) );
      const auto& bob_reward_balance = db->get_account( "bob" );
      BOOST_REQUIRE( bob_reward_balance.reward_vesting_freezone.amount == bob_SST_balance.pending_vesting_value.amount );

      const auto& charlie_SST_balance = db->get< account_rewards_balance_object, by_name_liquid_symbol >( boost::make_tuple( "charlie", alice_symbol ) );
      const auto& charlie_reward_balance = db->get_account( "charlie" );
      BOOST_REQUIRE( charlie_reward_balance.reward_vesting_freezone.amount == charlie_SST_balance.pending_vesting_value.amount );

      const auto& dave_SST_balance = db->get< account_rewards_balance_object, by_name_liquid_symbol >( boost::make_tuple( "dave", alice_symbol ) );
      const auto& dave_reward_balance = db->get_account( "dave" );
      BOOST_REQUIRE( dave_reward_balance.reward_vesting_freezone.amount == dave_SST_balance.pending_vesting_value.amount );
...
BOOST_AUTO_TEST_CASE( SST_token_emissions )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing SST token emissions" );
      ACTORS( (creator)(alice)(bob)(charlie)(dan)(elaine)(fred)(george)(henry) )

      vest( freezone_INIT_MINER_NAME, "creator", ASSET( "100000.000 TESTS" ) );
      ...
      signed_transaction tx;
      BOOST_TEST_MESSAGE( " --- SST setup emissions" );
      SST_setup_emissions_operation emissions_op;
      emissions_op.control_account = "creator";
      emissions_op.emissions_unit.token_unit[ SST_DESTINATION_REWARDS ]      = 2;
      emissions_op.emissions_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] = 2;
      emissions_op.emissions_unit.token_unit[ SST_DESTINATION_VESTING ]      = 2;
      emissions_op.emissions_unit.token_unit[ "george" ]                     = 1;
      emissions_op.interval_seconds = SST_EMISSION_MIN_INTERVAL_SECONDS;
      emissions_op.emission_count   = 25;
      emissions_op.symbol = symbol;
      emissions_op.schedule_time  = db->head_block_time() + ( freezone_BLOCK_INTERVAL * 10 );
      emissions_op.lep_time       = emissions_op.schedule_time + ( freezone_BLOCK_INTERVAL * freezone_BLOCKS_PER_DAY * 2 );
      emissions_op.rep_time       = emissions_op.lep_time + ( freezone_BLOCK_INTERVAL * freezone_BLOCKS_PER_DAY * 2 );
      ...
      SST_setup_emissions_operation emissions_op2;
      emissions_op2.control_account = "creator";
      emissions_op2.emissions_unit.token_unit[ SST_DESTINATION_REWARDS ]      = 1;
      emissions_op2.emissions_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] = 1;
      emissions_op2.emissions_unit.token_unit[ SST_DESTINATION_VESTING ]      = 1;
      emissions_op2.emissions_unit.token_unit[ "george" ]                     = 1;
      emissions_op2.interval_seconds = SST_EMISSION_MIN_INTERVAL_SECONDS;
      emissions_op2.emission_count   = 25;
      emissions_op2.symbol = symbol;
      emissions_op2.schedule_time  = emissions_op.schedule_time + ( emissions_op.interval_seconds * ( emissions_op.emission_count - 1 ) ) + SST_EMISSION_MIN_INTERVAL_SECONDS;
      emissions_op2.lep_time       = emissions_op2.schedule_time + ( freezone_BLOCK_INTERVAL * freezone_BLOCKS_PER_DAY * 2 );
      emissions_op2.rep_time       = emissions_op2.lep_time + ( freezone_BLOCK_INTERVAL * freezone_BLOCKS_PER_DAY * 2 );
      ...
      SST_setup_emissions_operation emissions_op3;
      emissions_op3.control_account = "creator";
      emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_REWARDS ]      = 1;
      emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] = 1;
      emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_VESTING ]      = 1;
      emissions_op3.emissions_unit.token_unit[ "george" ]                     = 1;
      emissions_op3.interval_seconds = SST_EMISSION_MIN_INTERVAL_SECONDS;
      emissions_op3.emission_count   = SST_EMIT_INDEFINITELY;
      emissions_op3.symbol = symbol;
      emissions_op3.schedule_time  = emissions_op2.schedule_time + ( emissions_op2.interval_seconds * ( emissions_op2.emission_count - 1 ) ) + SST_EMISSION_MIN_INTERVAL_SECONDS;
      ...
      freezone::plugins::rc::rc_plugin_skip_flags rc_skip;
      ...
      appbase::app().get_plugin< freezone::plugins::rc::rc_plugin >().set_rc_plugin_skip_flags( rc_skip );

      freezone::plugins::rc::delegate_to_pool_operation del_op;
      ...
      custom_op.json = fc::json::to_string( freezone::plugins::rc::rc_plugin_operation( del_op ) );
      ...
      BOOST_TEST_MESSAGE( " --- SST token emissions" );
      ...
      share_type vesting = token.total_vesting_fund_SST;
      ...
      for ( uint32_t i = 0; i <= ( emissions_op.emission_count - 1 ); i++ )
      {
         ...
         share_type new_rewards = new_token_supply * emissions_op.emissions_unit.token_unit[ SST_DESTINATION_REWARDS ] / emissions_op.emissions_unit.token_unit_sum();
         share_type new_market_maker = new_token_supply * emissions_op.emissions_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] / emissions_op.emissions_unit.token_unit_sum();
         share_type new_vesting = new_token_supply * emissions_op.emissions_unit.token_unit[ SST_DESTINATION_VESTING ] / emissions_op.emissions_unit.token_unit_sum();
         ...
         vesting += new_vesting;
         BOOST_REQUIRE( approximately_equal( token.total_vesting_fund_SST, vesting ) );
      ...
      for ( uint32_t i = 0; i <= ( emissions_op2.emission_count - 1 ); i++ )
      {
         ...
         share_type new_rewards = new_token_supply * emissions_op2.emissions_unit.token_unit[ SST_DESTINATION_REWARDS ] / emissions_op2.emissions_unit.token_unit_sum();
         share_type new_market_maker = new_token_supply * emissions_op2.emissions_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] / emissions_op2.emissions_unit.token_unit_sum();
         share_type new_vesting = new_token_supply * emissions_op2.emissions_unit.token_unit[ SST_DESTINATION_VESTING ] / emissions_op2.emissions_unit.token_unit_sum();
      ...
      BOOST_TEST_MESSAGE( " --- SST token emissions catch-up logic" );

      share_type new_token_supply = emissions_op3.lep_abs_amount;
      share_type new_rewards = new_token_supply * emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_REWARDS ] / emissions_op3.emissions_unit.token_unit_sum();
      share_type new_market_maker = new_token_supply * emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] / emissions_op3.emissions_unit.token_unit_sum();
      share_type new_vesting = new_token_supply * emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_VESTING ] / emissions_op3.emissions_unit.token_unit_sum();
      ...
      emission_time += SST_EMISSION_MIN_INTERVAL_SECONDS * 2;
      generate_blocks( emission_time );
      generate_blocks( 3 );

      new_token_supply = ( emissions_op3.lep_abs_amount * 2 );

      new_rewards = new_token_supply * emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_REWARDS ] / emissions_op3.emissions_unit.token_unit_sum();
      new_market_maker = new_token_supply * emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_MARKET_MAKER ] / emissions_op3.emissions_unit.token_unit_sum();
      new_vesting = new_token_supply * emissions_op3.emissions_unit.token_unit[ SST_DESTINATION_VESTING ] / emissions_op3.emissions_unit.token_unit_sum();
      ...
      emission_time += SST_EMISSION_MIN_INTERVAL_SECONDS * 6;
      generate_blocks( emission_time );
      generate_blocks( 11 );
      ...
      BOOST_TEST_MESSAGE( " --- SST token emissions do not emit passed max supply" );
      ...
      transfer.to = freezone_NULL_ACCOUNT;
      transfer.amount = asset( georges_burn, symbol );

      BOOST_TEST_MESSAGE( " --- SST token burn" );
      ...
      BOOST_REQUIRE( approximately_equal( db->get_balance( db->get_account( "george" ), symbol ).amount, george_share ) );
      BOOST_REQUIRE( db->get_balance( freezone_NULL_ACCOUNT, symbol ).amount == 0 );
      BOOST_REQUIRE( approximately_equal( token.current_supply, supply ) );
      ...
      BOOST_TEST_MESSAGE( " --- SST token emissions re-emit after token burn" );
...
BOOST_AUTO_TEST_CASE( SST_without_ico )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing SST without ICO" );
      ...
      BOOST_TEST_MESSAGE( " --- SST setup and token emission" );
      signed_transaction tx;
      SST_setup_operation setup_op;
      ...
      SST_setup_emissions_operation token_emission_op;
      ...
      token_emission_op.schedule_time = setup_op.launch_time + freezone_BLOCK_INTERVAL;
      token_emission_op.emission_count = 1;
      token_emission_op.interval_seconds = SST_EMISSION_MIN_INTERVAL_SECONDS;
...
BOOST_AUTO_TEST_CASE( SST_action_delay )
{ try {
    ...
    * cascade, they would use head_block_time + freezone_BLOCK_INTERVAL.
    *
    * In this case the head block is at time 1 and the only pending required
    * action is an ico_launch_action for SST1 at time 4.
    *
    * We then push the setup for SST2 and create a block at time 8. In this
    * time, SST 1 should complete its ICO, evaulate the ICO, and
    * launch, happening at times 4 and 5.
    *
    * When we push the SST2 setup ops, we want to push an ico_launch_action for
    * SST2 at time3, an SST_ico_evaulation_action at time 8 and finally
    *
    * In the buggy code, block generation does not update the head block time
    * during generation.
    *
    * We evaluate the SST2 op and push ico_launch_action at time 3.
    *
    * The required actions queue is:
    *
    * SST_ico_launch_action, SST2: time 3
    * SST_ico_evaluation_action, SST1: time 4
    *
    * When evaluating actions, we will add the following actions in this order:
    *
    * SST_token_launch_action, SST1: time 5
    * SST_ico_evaluation_action, SST2: time 8
    * SST_token_launch_action, SST2: time 9
    *
    ...
    *
    * SST_ico_evaluation_action, SST2: time 9
    * SST_token_launch_action, SST2: time 9
    * SST_token_launch_action, SST1: time 9
    *
   ...
   SST_create_operation create_op;
   create_op.control_account  = "alice";
   create_op.SST_creation_fee = db->get_dynamic_global_properties().SST_creation_fee;
   create_op.symbol           = get_new_SST_symbol( 3, db );
   create_op.precision        = create_op.symbol.decimals();

   SST_setup_operation setup_op;
   setup_op.control_account         = "alice";
   setup_op.symbol                  = create_op.symbol;
   setup_op.contribution_begin_time = db->head_block_time() + freezone_BLOCK_INTERVAL; // time 1
   setup_op.contribution_end_time   = setup_op.contribution_begin_time + 3 * freezone_BLOCK_INTERVAL; // time 4
   setup_op.freezone_satoshi_min       = 0;
   setup_op.min_unit_ratio          = 50;
   setup_op.max_unit_ratio          = 100;
   setup_op.max_supply              = freezone_MAX_SHARE_SUPPLY;
   setup_op.launch_time             = setup_op.contribution_end_time + freezone_BLOCK_INTERVAL; // time 5
   ...
   create_op.symbol                 = get_new_SST_symbol( 6, db );
   create_op.precision              = create_op.symbol.decimals();
   setup_op.symbol                  = create_op.symbol;
   setup_op.contribution_begin_time = db->head_block_time() + 2 * freezone_BLOCK_INTERVAL; // time 3
   setup_op.contribution_end_time   = setup_op.contribution_begin_time + 5 * freezone_BLOCK_INTERVAL; // time 8
   setup_op.launch_time             = setup_op.contribution_end_time + freezone_BLOCK_INTERVAL; // time 9
