...
target_link_libraries( {plugin_provider}_{plugin_name} freezone_app freezone_chain freezone_protocol )
...
{{
   public:
      {plugin_name}_api( const freezone::app::api_context& ctx );
...
class {plugin_name}_plugin : public freezone::app::plugin
{{
   public:
      {plugin_name}_plugin( freezone::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <freezone/app/api_context.hpp>
#include <freezone/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( freezone::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      freezone::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( freezone::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const freezone::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( freezone::app::application& app );
      ...
      freezone::app::application& _app;
      boost::signals2::scoped_connection _applied_block_conn;
}};

{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( freezone::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( freezone::app::application* app )
   : plugin(app)
{{
...
freezone_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (freezone for plugins developed by freezone)")
