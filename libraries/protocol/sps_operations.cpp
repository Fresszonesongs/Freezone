...
void create_proposal_operation::validate()const
{
   ...
   FC_ASSERT( daily_pay.symbol.asset_num == freezone_ASSET_NUM_SBD, "Daily pay should be expressed in SBD");

   FC_ASSERT( !subject.empty(), "subject is required" );
   FC_ASSERT( subject.size() <= freezone_PROPOSAL_SUBJECT_MAX_LENGTH, "Subject is too long");
...
void update_proposal_votes_operation::validate()const
{
   ...
   FC_ASSERT(proposal_ids.size() <= freezone_PROPOSAL_MAX_IDS_NUMBER);
}
