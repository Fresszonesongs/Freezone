...
{
   static_assert( freezone_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
asset_symbol_type asset_symbol_type::from_nai_string( const char* p, uint8_t decimal_places )
{
   try
   {
      ...
      FC_ASSERT( std::strlen( p ) == freezone_ASSET_SYMBOL_NAI_STRING_LENGTH - 1, "Incorrect NAI string length" );
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   FC_ASSERT( (nai_data_digits >= SST_MIN_NAI) & (nai_data_digits <= SST_MAX_NAI), "NAI out of range" );
   FC_ASSERT( nai_check_digit == damm_checksum_8digit(nai_data_digits), "Invalid check digit" );

   switch( nai_data_digits )
   {
      case freezone_NAI_freezone:
         FC_ASSERT( decimal_places == freezone_PRECISION_freezone );
         return freezone_ASSET_NUM_freezone;
      case freezone_NAI_SBD:
         FC_ASSERT( decimal_places == freezone_PRECISION_SBD );
         return freezone_ASSET_NUM_SBD;
      case freezone_NAI_VESTS:
         FC_ASSERT( decimal_places == freezone_PRECISION_VESTS );
         return freezone_ASSET_NUM_VESTS;
      default:
         FC_ASSERT( decimal_places <= freezone_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
         return (nai_data_digits << freezone_NAI_SHIFT) | SST_ASSET_NUM_CONTROL_MASK | decimal_places;
   }
}
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      case freezone_ASSET_NUM_freezone:
         nai_data_digits = freezone_NAI_freezone;
         break;
      case freezone_ASSET_NUM_SBD:
         nai_data_digits = freezone_NAI_SBD;
         break;
      case freezone_ASSET_NUM_VESTS:
         nai_data_digits = freezone_NAI_VESTS;
         break;
      default:
         FC_ASSERT( space() == SST_nai_space );
         nai_data_digits = (asset_num >> freezone_NAI_SHIFT);
   }
...
bool asset_symbol_type::is_vesting() const
{
   switch( space() )
   {
      ...
      case SST_nai_space:
         ...
         return asset_num & SST_ASSET_NUM_VESTING_MASK;
      default:
         FC_ASSERT( false, "Unknown asset symbol" );
   }
}
...
asset_symbol_type asset_symbol_type::get_paired_symbol() const
{
   switch( space() )
   {
      case legacy_space:
      {
         ...
         {
            case freezone_ASSET_NUM_freezone:
               return from_asset_num( freezone_ASSET_NUM_VESTS );
            case freezone_ASSET_NUM_SBD:
               return *this;
            case freezone_ASSET_NUM_VESTS:
               return from_asset_num( freezone_ASSET_NUM_freezone );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
      case SST_nai_space:
         ...
         auto paired_asset_num = asset_num ^ ( SST_ASSET_NUM_VESTING_MASK );
...
asset_symbol_type asset_symbol_type::get_vesting_symbol()const
{
   switch( space() )
   {
      ...
      case SST_nai_space:
      {
         return from_asset_num( asset_num | SST_ASSET_NUM_VESTING_MASK );
      }
      default:
...
void asset_symbol_type::validate()const
{
   switch( asset_num )
   {
      ...
      default:
      {
         uint32_t nai_data_digits = (asset_num >> freezone_NAI_SHIFT);
         uint32_t nai_1bit = (asset_num & SST_ASSET_NUM_CONTROL_MASK);
         uint32_t nai_decimal_places = (asset_num & SST_ASSET_NUM_PRECISION_MASK);
         FC_ASSERT( (nai_data_digits >= SST_MIN_NAI) &
                    (nai_data_digits <= SST_MAX_NAI) &
                    (nai_1bit == SST_ASSET_NUM_CONTROL_MASK) &
                    (nai_decimal_places <= freezone_ASSET_MAX_DECIMALS),
                    "Cannot determine space for asset ${n}", ("n", asset_num) );
      }
   }
   // this assert is duplicated by above code in all cases
   // FC_ASSERT( decimals() <= freezone_ASSET_MAX_DECIMALS );
}
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= freezone_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(freezone_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( freezone_MAX_SATOSHIS, quote); }
      ...
      { try {
         ...
         else
         {
            ...
            if ( ( base.symbol == SBD_SYMBOL && quote.symbol == freezone_SYMBOL ) || ( base.symbol == freezone_SYMBOL && quote.symbol.space() == asset_symbol_type::SST_nai_space ) )
...
namespace fc {
   ...
   void from_variant( const fc::variant& var, freezone::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = freezone::protocol::asset_symbol_type::from_nai_string( v_object[ ASSET_NAI_KEY ].as< std::string >().c_str(), v_object[ ASSET_PRECISION_KEY ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
