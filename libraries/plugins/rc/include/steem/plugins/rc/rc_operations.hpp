...
using freezone::protocol::extensions_type;
...
/**
 * Delegate SP or a vesting SST to a pool.
 *
 ...
 * RC for to_pool.
 *
 * Each SST delegated to an account forms its own pool.  Each account has only 3
 * slots for receiving delegation (`freezone_RC_MAX_OUTDEL_SLOTS`).
 */
struct delegate_to_pool_operation : freezone::protocol::base_operation
{
...
/**
 * Delegate SP or a vesting SST from a pool to a user.
 *
 ...
 */
struct delegate_drc_from_pool_operation : freezone::protocol::base_operation
{
 ...
 * the existing delegator, the account itself, or (in the case of slot 1, freezone_RC_RECOVERY_SLOT_NUM)
 * the slot may also be set by the account's recovery partner.
 */
struct set_slot_delegator_operation : freezone::protocol::base_operation
{
   account_name_type      from_pool;
   account_name_type      to_account;
   int8_t                 to_slot = 0;
...
freezone_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, delegate_to_pool );
freezone_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, delegate_drc_from_pool );
freezone_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, set_slot_delegator );

} } } // freezone::plugins::rc

FC_REFLECT( freezone::plugins::rc::delegate_to_pool_operation,
   (from_account)
   (to_pool)
   (amount)
   (extensions)
   )
FC_REFLECT( freezone::plugins::rc::delegate_drc_from_pool_operation,
   (from_pool)
   (to_account)
   (to_slot)
   (asset_symbol)
...
FC_REFLECT( freezone::plugins::rc::set_slot_delegator_operation,
   (from_pool)
   (to_account)
   (to_slot)
   (signer)
...
freezone_DECLARE_OPERATION_TYPE( freezone::plugins::rc::rc_plugin_operation )
FC_REFLECT_TYPENAME( freezone::plugins::rc::rc_plugin_operation )
