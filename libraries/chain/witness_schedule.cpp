...
using freezone::chain::util::rd_system_params;
using freezone::chain::util::rd_user_params;
using freezone::chain::util::rd_validate_user_params;
...
void reset_virtual_schedule_time( database& db )
{
   ...
   for( const auto& witness : idx )
   {
      db.modify( witness, [&]( witness_object& wobj )
      {
         ...
         wobj.virtual_scheduled_time = freezone_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wobj.votes.value+1);
      } );
   }
}
...
void update_median_witness_props( database& db )
{
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      int64_t min_decay = (fc::uint128( median_decay ) * freezone_DECAY_BACKSTOP_PERCENT / freezone_100_PERCENT).to_int64();
...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( freezone_MAX_WITNESSES );
   ...
   for( auto itr = widx.begin();
        ...
        ++itr )
   {
      if( db.has_hardfork( freezone_HARDFORK_0_14__278 ) && (itr->signing_key == public_key_type()) )
         continue;
   ...
   while( mitr != pow_idx.end() && selected_miners.size() < wso.max_miner_witnesses )
   {
      ...
      if( selected_voted.find(mitr->id) == selected_voted.end() )
      {
         ...
         if( !( db.has_hardfork( freezone_HARDFORK_0_14__278 ) && db.get_witness( mitr->owner ).signing_key == public_key_type() ) )
         {
   ...
   for( auto witness_count = selected_voted.size() + selected_miners.size();
        sitr != schedule_idx.end() && witness_count < freezone_MAX_WITNESSES;
        ++sitr )
   {
      new_virtual_time = sitr->virtual_scheduled_time; /// everyone advances to at least this time
      processed_witnesses.push_back(sitr);

      if( db.has_hardfork( freezone_HARDFORK_0_14__278 ) && sitr->signing_key == public_key_type() )
   ...
   {
      auto new_virtual_scheduled_time = new_virtual_time + freezone_VIRTUAL_SCHEDULE_LAP_LENGTH2 / ((*itr)->votes.value+1);
   ...
   size_t expected_active_witnesses = std::min( size_t(freezone_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("freezone_MAX_WITNESSES",freezone_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );

   auto majority_version = wso.majority_version;

   if( db.has_hardfork( freezone_HARDFORK_0_5__54 ) )
   {
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < freezone_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % freezone_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
      if( db.has_hardfork(freezone_HARDFORK_0_4) )
      {
      ...
      /// only use vote based scheduling after the first 1M freezone is created or if there is no POW queued
      if( props.num_pow_witnesses == 0 || db.head_block_num() > freezone_START_MINER_VOTING_BLOCK )
      {
         const auto& widx = db.get_index<witness_index>().indices().get<by_vote_name>();

         for( auto itr = widx.begin(); itr != widx.end() && (active_witnesses.size() < (freezone_MAX_WITNESSES-2)); ++itr )
         {
            if( itr->pow_worker )
               continue;
         ...
         if( sitr != schedule_idx.end() )
         {
            ...
            db.modify( *sitr, [&]( witness_object& wo )
            {
               ...
               if( db.has_hardfork( freezone_HARDFORK_0_2 ) )
                  wo.virtual_scheduled_time += freezone_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wo.votes.value+1);
               else
                  wo.virtual_scheduled_time += freezone_VIRTUAL_SCHEDULE_LAP_LENGTH / (wo.votes.value+1);
            } );
         }
      }
      ...
      if( props.num_pow_witnesses > freezone_MAX_WITNESSES )
      {
         if( itr != pow_idx.end() )
         {
            db.modify( *itr, [&](witness_object& wit )
      ...
      while( itr != pow_idx.end() )
      {
         ...
         if( db.head_block_num() > freezone_START_MINER_VOTING_BLOCK || active_witnesses.size() >= freezone_MAX_WITNESSES )
            break;
         ++itr;
      }
      ...
      /*
         ...
         for( const string& w : active_witnesses )
            ...
            */
         // active witnesses has exactly freezone_MAX_WITNESSES elements, asserted above
         for( size_t i = 0; i < active_witnesses.size(); i++ )
         {
            _wso.current_shuffled_witnesses[i] = active_witnesses[i];
         }
